                ...
                <div class="w-full lg:w-5/6 m-10">
                    ...
                    <div v-if="show"  class=" mt-5  max-w-9xl w-full bg-green-500 shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5 divide-x divide-gray-200 ">
                        <div class="w-0 flex-1 flex items-center p-4">
                            <div class="w-full">
                                <p class="mt-1 text-sm text-white">Update with success</p>
                            </div>
                        </div>
                    </div>
                    <div class="bg-white shadow-md rounded my-6">
                        <table class="min-w-max w-full table-auto">
                            ...
                            <tr v-for="(room ,index) in hotel_mapping " class="border-b border-gray-200 hover:bg-gray-100">
                                <td class="py-3 px-6 text-left whitespace-nowrap">
                                    <div class="flex items-center">
                                        <span class="font-medium">{{room.categorie_name}}</span>
                                    </div>
                                </td>
                    ...
                    <button  @click="save" type="button" class="inline-flex items-center mt-2  px-6 py-3 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Save Change</button>
                </div>
            </div>
        </div>
    </AppLayout>
...
export default {
    ...
    methods: {
        save(){
            axios.post('/hotel-mapping-hotel/update', {
                rooms: this.hotel_mapping ,
            }).then( (response ) =>  {
                    this.errors = {};
                    this.message = '';
                    this.show = true ;
